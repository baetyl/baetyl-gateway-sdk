...
func (m *Map) Collect() ([]byte, error) {
	res, err := m.read()
	if err != nil {
		m.log.Error("failed to connectors data from slave", "slave id", m.s.cfg.Id, "config", m.cfg, "error", err)
		if err1 := m.s.client.Reconnect(); err1 != nil {
			m.log.Error("failed to reconnect", "slave id", m.s.cfg.Id, "error", err1)
			return nil, err

		}
		m.log.Info("reconnected successfully", "slave id", m.s.cfg.Id)
		// try to read again
		res, err = m.read()
		if err != nil {
			return nil, err
...
func (m *Map) Parse(data []byte) (any, error) {
...
func parse(reader io.Reader, cfg MapConfig) (res any, err error) {
